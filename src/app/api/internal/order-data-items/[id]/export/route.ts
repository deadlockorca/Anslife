...
function canActorExportDataItem(
...
export async function GET(
  ...
  try {
    ...
    const rateLimit = consumeRateLimit({
      namespace: 'data_item_export',
      key: `${actor.userId}:${clientIp}`,
      max: readRateLimitEnv('APP_RATE_LIMIT_EXPORT_MAX', 60),
      windowMs: readRateLimitEnv('APP_RATE_LIMIT_EXPORT_WINDOW_SECONDS', 600) * 1000,
    });
    if (!rateLimit.allowed) {
      return createRateLimitResponse(
        'Bạn đang export dữ liệu quá nhanh. Vui lòng thử lại sau.',
        rateLimit,
      );
    }
    ...
    if (!canActorExportDataItem(actor, item)) {
      return applyRateLimitHeaders(NextResponse.json(
        { ok: false, code: 'forbidden', message: 'Bạn không có quyền export dữ liệu này.' },
        { status: 403 },
      ), rateLimit);
    }
    ...
    const filename = sanitizeFilename(`${item.orderNo}-${item.dataType}-item-${item.id}.json`);
    return applyRateLimitHeaders(new NextResponse(JSON.stringify({ ok: true, item }, null, 2), {
      status: 200,
      headers: {
    ...
    console.error('[API][internal][order-data-items/:id/export][GET] Failed:', error);
    return NextResponse.json(
      { ok: false, code: 'internal_error', message: 'Không thể export dữ liệu.' },
      { status: 500 },
    );
  }
}
