...
`.trim()};function r(n){return h[n]??null}function o(n){return n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function u(n,c){const t=h[n];if(!t)return null;const g=o(c),s=new RegExp(`<section\\s+id="${g}"[^>]*>[\\s\\S]*?<\\/section>`,"i"),i=t.match(s);if(!i)return null;const a=t.match(/<figure class="ai-banner">[\s\S]*?<\/figure>/i),l=t.match(/<p class="ai-intro">[\s\S]*?<\/p>/i);return`
<div class="ai-content">
  ${a?.[0]??""}
  ${l?.[0]??""}
  ${i[0]}
</div>
`.trim()}export{u as a,r as g};
